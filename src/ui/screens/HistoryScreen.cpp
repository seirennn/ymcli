    ...
    component_ = ftxui::Renderer([this] {
        ...
        for (size_t i = 0; i < items_.size(); ++i) {
            ...
            auto el = ftxui::hbox({
                ...
                ftxui::text(item.artist) | ftxui::color(Theme::TextSecondary) | ftxui::flex,
                ftxui::text(item.time) | ftxui::color(Theme::TextTertiary)
            });
        ...
        return ftxui::vbox({
            Theme::heading("Play History"),
            ftxui::text("[Enter] Play  [f] Favorite") | ftxui::color(Theme::DimAccent),
...
void HistoryScreen::SetHistory(const std::vector<HistoryItem>& items) { items_ = items; selected_ = 0; }
